            ...
            while self.__is_subscription_active:
                ...
                try:
                    ...
                    if message.type in self.__handlers:
                        logging.debug(
                            f"Calling handler for {message.type}, full message:\n",
                            json.dumps(message.to_dict(), indent=2),
                        )
                        if asyncio.iscoroutine(self.__handlers[message.type]):
                    ...
                    logging.debug(
                        f'No messages arrived during the pull_wait_timeout_in_secs ({self.__pull_wait_timeout_in_secs}). "Re-arming" fetch'
                    )
                    pass
